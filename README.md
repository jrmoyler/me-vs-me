...
- **Arcade ladder:** eight reflections dealt from the other nineteen identities with the roles mixed, then a final against **your shadow**: a mirror match of your own fighter. A destruction bonus follows wins three and six, and each win or loss ends with a one-line quote from the winner. Turn on **FULL CIRCLE** on the stage or route screen to face all nineteen before the shadow; the choice is saved. Two rounds win each match. Defeat offers a ten-second continue.
...
|---|---|---|
| Zoner (projectile zoning) | Iron Will, Shadow Self, Nexus, Zenith | POWER pushes 18 px further on block; jumping LP reach +8 |
| Rushdown (command dash / rising launcher pressure) | Hataalii, Green Light, Fast Lane, Old Soul, Glyph | Throw range +12; jab recovery −0.02 s (startup unchanged) |
| Counter (counter hit specialist) | Quiet Luxury, Civic Sage, Quilt | First punish after a successful block is a counter hit; POWER startup +0.04 s, damage +2 |
| Grappler (close-range throws) | Varsity | Throw damage 16, range +18; wakeup window +0.04 s |
| Balanced (long-range footsies) | After Hours, Blueprint, Wild Card, Hybrid, Binary, Aether, Gaia | None |

Varsity's Overtime Elbow is a travel/elbow power. The kit brief grouped it with rushdown or counter, but it was made the grappler so the role has a fighter. The rushdown jab change lowers the smallest recovery-plus-next-startup gap from 0.41 s to 0.39 s. That is still above the 0.36 s jab hitstun, and tests assert no link combos for any kit.
...
Vercel runs the test suite before building `dist` using the included configuration. The Vite build emits content-hashed `assets/index-*.js` and `.css`, which `vercel.json` serves as immutable. Character and arena art stays at stable paths and revalidates. `npm run package:offline` produces `release/Me-vs-Me.html` (**70.1 MB**, last measured). It embeds every combat atlas, motion atlas, portrait and arena as data URIs, so it is not part of the Vercel build. Settings (including remapped keys and FULL CIRCLE) and local records persist in browser storage when available. No game backend, account or network call is required. `render-arenas.mjs` and `qa-screens.mjs` use Playwright's Chromium; a global `playwright` install is found automatically, or point `PLAYWRIGHT_MODULE` at one. `qa-screens.mjs` also checks that the built title has the Versus button and the help modal has the THROW hint.
