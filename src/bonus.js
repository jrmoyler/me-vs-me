...
export const BONUS_STRIKES = Object.freeze({
  j: 5,
  k: 8,
  l: 12,
  u: 6,
...
export function startBonus({
  ...
  @media(max-height:480px){.mvm-bonus{gap:5px;padding:8px}.mvm-bonus-stage{height:38vh;min-height:125px}.mvm-bonus h1{font-size:22px}.mvm-bonus-controls{grid-template-columns:repeat(6,minmax(60px,100px));gap:4px}.mvm-bonus button{min-height:36px}}
  </style><div class="mvm-bonus"><h1>BONUS STAGE</h1><p>BREAK THE REFLECTION · Strike the crystal before time runs out.</p><div class="mvm-bonus-hud"><span>SCORE <b data-score>00000</b></span><span>TIME <b data-time>20</b></span></div><div class="mvm-bonus-stage"><div class="mvm-bonus-fighter" role="img" aria-label="${esc(character.name)}"></div><div class="mvm-bonus-target" role="img" aria-label="Reflection crystal">ME</div></div><meter min="0" max="1" value="1" aria-label="Crystal integrity"></meter><div class="mvm-bonus-feedback" role="status" aria-live="polite">READY? STRIKE!</div><div class="mvm-bonus-controls">${[
    ["j", "LIGHT P"],
    ["k", "MEDIUM P"],
    ["l", "HEAVY P"],
    ["u", "LIGHT K"],
    ...
    .map(
      ([key, label]) =>
        `<button data-strike="${key}" aria-label="${label} (${key.toUpperCase()})">${label} · ${key.toUpperCase()}</button>`,
    )
  ...
  function finish(skipped = false) {
    ...
    finished = true;
    cleanup();
    onEnd?.({ score, destroyed: integrity <= 0, skipped });
  }
  function strike(key) {
    ...
    if (
      ...
      return;
    const damage = BONUS_STRIKES[key];
    nextHit = now + 130 + damage * 9;
    const row = ["j", "k", "l", "u", "i", "o"].indexOf(key);
    const y = `${(row / 6) * 100}%`;
    fighter.dataset.attacking = "true";
    fighter.style.backgroundPosition = `66.6667% ${y}`;
    if (!settings.reducedMotion && fighter.animate)
      fighter.animate(
        [
          { backgroundPosition: `0% ${y}`, offset: 0 },
          { backgroundPosition: `33.3333% ${y}`, offset: 0.2 },
          { backgroundPosition: `66.6667% ${y}`, offset: 0.43 },
          { backgroundPosition: `100% ${y}`, offset: 0.8 },
        ],
        { duration: 130 + damage * 9, easing: "steps(1)" },
      );
    integrity = Math.max(0, integrity - damage);
    score += damage * 25;
    meter.value = integrity / 240;
    target.dataset.damaged = String(integrity < 160);
    feedback.textContent = `${key === "l" || key === "o" ? "CRUSH!" : "HIT!"} +${damage * 25}`;
    if (!settings.reducedMotion && target.animate)
      target.animate(
        [
          { translate: "-5px 0", filter: "brightness(2)" },
          { translate: "5px 0" },
    ...
    if (integrity === 0) {
      score += Math.ceil(remaining) * 100;
      target.dataset.broken = "true";
      feedback.textContent = "PERFECT! REFLECTION SHATTERED";
      buttons.forEach((b) => (b.disabled = true));
      finishTimer = setTimeout(() => finish(), 1200);
    }
    root.querySelector("[data-score]").textContent = String(score).padStart(
      5,
      "0",
    );
  }
  ...
  buttons.forEach((button) =>
    button.addEventListener("click", () => strike(button.dataset.strike)),
  );
  root
    .querySelector(".mvm-bonus-exit")
    .addEventListener("click", () => finish(true));
  interval = setInterval(() => {
    const now = performance.now(),
      elapsed = (now - last) / 1000;
    last = now;
    if (now >= nextHit && fighter.dataset.attacking === "true") {
      fighter.dataset.attacking = "false";
      fighter.style.backgroundPosition = "0% 0%";
    }
    if (finished || document.hidden || integrity <= 0) return;
    remaining = Math.max(0, remaining - elapsed);
    root.querySelector("[data-time]").textContent = String(
      Math.ceil(remaining),
    ).padStart(2, "0");
    if (remaining <= 0) {
