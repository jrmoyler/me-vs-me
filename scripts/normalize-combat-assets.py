"""Pack generated four-column combat and motion art; preserve source alpha.
...
for kind,rows,suffix in [('fighters',7,'combat'),('motions',6,'motion')]:
    for identity,path in sources.get(kind,{}).items():
        ...
        for number,bb in enumerate(ndimage.find_objects(labels),1):
            ...
            if yy.stop-yy.start>340:
                boundary=round((yy.start+yy.stop)/2/(h/rows))*(h/rows)
            ...
            for top,bottom in sections:
                row=min(rows-1,int((top+bottom)/2/(h/rows)));col=min(3,int((xx.start+xx.stop)/2/(w/4)))
        ...
        out=Image.new('RGBA',(1280,rows*320))
        ...
        destination=root/f'public/assets/characters/{identity}-{suffix}.png'
...
for identity,path in sources.get('arenas',{}).items():
