...
const result=await build({entryPoints:['src/main.js'],bundle:true,format:'iife',minify:true,write:false,outfile:'offline.js',loader:{'.css':'css'},external:['/assets/*']});
...
for(const folder of ['characters','arenas']){
 for(const name of await readdir(`public/assets/${folder}`)){
  ...
  css=css.split(`/assets/${folder}/${name}`).join(url);
 }
}
